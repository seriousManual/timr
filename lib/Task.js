...
Task.prototype._calculateNextRun = function(initial) {
    ...
    if(this._from && now.valueOf() < this._from.valueOf()) {
        ...
        if(initial) {
            ...
            return now.add(this._interval, this._modifier);
        }
    }
};
...
/**
 * sets the execution modifier to month
 *
 * @type {Function}
 */
Task.prototype.months = Task.prototype.month = function () {
    this._modifier = 'months';
...
Task.prototype.to = function(to) {
    this._validateTimestamp(to);
    this._to = moment(to);

    return this;
};

Task.prototype._validateTimestamp = function(timeStamp) {
